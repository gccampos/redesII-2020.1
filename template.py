...
def codePacket(originalPacket):
    ...
    ##
    codedPacket = list(originalPacket)
    return codedPacket
...
def help(selfName):
    ...
    sys.stderr.write("\t" + selfName + " <tam_pacote> <reps> <prob. erro> <nome_csv_resultados>\n\n")
    ...
    sys.stderr.write("\t- <nome_csv_resultados>: nome do arquivo csv que armazenará os resultados\n")
    sys.exit(1)
...
colunas = ['transmissoesSimuladas', 'numBitsTransmitidos', 'bitsErradosInseridos', 'taxaErroBitsAntesDecod',
               'numBitsCorrompidosPosDecod', 'taxaErroBitsPosDecod', 'pacotesCorrompidos', 'taxaErroPacotes']
writer = csv.DictWriter(csvFile, fieldnames=colunas)
writer.writeheader()

with csvFile:
    writer.writerow({colunas[0]: format(reps),
                     colunas[1]: format(reps * packetLength * 8),
                     colunas[2]: format(totalInsertedErrorCount),
                     colunas[3]: format(float(totalInsertedErrorCount) / float(reps * len(codedPacket)) * 100.0),
                     colunas[4]: format(totalBitErrorCount),
                     colunas[5]: format(float(totalBitErrorCount) / float(reps * packetLength * 8) * 100.0),
                     colunas[6]: format(totalPacketErrorCount),
                     colunas[7]: format(float(totalPacketErrorCount) / float(reps) * 100.0)
                     })
...
f.write('\n')
f.write('\nNumero de transmissoes simuladas: {0:d}\n'.format(reps))
f.write('Numero de bits transmitidos: {0:d}\n'.format(reps * packetLength * 8))
f.write('Numero de bits errados inseridos: {0:d}\n'.format(totalInsertedErrorCount))
f.write('Taxa de erro de bits (antes da decodificacao): {0:.2f}%\n'.format(
    float(totalInsertedErrorCount) / float(reps * len(codedPacket)) * 100.0))
f.write('Numero de bits corrompidos apos decodificacao: {0:d}\n'.format(totalBitErrorCount))
f.write('Taxa de erro de bits (apos decodificacao): {0:.2f}%\n'.format(
